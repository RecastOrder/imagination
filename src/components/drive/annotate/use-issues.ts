...
export function useIssues(fileId: string) {
  ...
  const raise = useCallback(
    async (input: { mark: Mark; title: string; scale?: number; fileName: string }) => {
  ...
  const reply = useCallback(async (id: string, text: string) => upsert(await call(`/api/issues/${id}`, "POST", { text })), [upsert])
  const setStatus = useCallback(
    async (id: string, status: Issue["status"]) => upsert(await call(`/api/issues/${id}`, "PATCH", { status })),
    [upsert],
  )

  return { issues, loaded, raise, reply, setStatus }
}
