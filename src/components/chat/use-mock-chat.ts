...
import { mockRespond } from "@/lib/chat/mock-responder"
 ...
 */
export function useMockChat(respond: (q: string) => { status: string; blocks: Block[] } = mockRespond) {
  ...
  const send = useCallback(
    (text: string) => {
      const q = text.trim()
      if (!q || busy) return
      const { status, blocks } = respond(q)
      const aid = uid()
      setMessages((ms) => [
        ...ms,
        { id: uid(), role: "user", blocks: [{ type: "text", content: q }] },
        { id: aid, role: "assistant", blocks: [], streaming: true, status },
      ])
