...
import { listRounds } from "@/lib/server/condition-rounds"
...
export default async function ProjectPage({ params }: PageProps<"/projects/[id]">) {
  ...
  return (
    <Suspense fallback={<PageSkeleton variant="list" />}>
      <ProjectView initial={p} initialRefs={listRefs(p.id)} issues={listIssuesInScope(`proj:${p.id}`)} rounds={listRounds(p)} access={accessOf(p, user.email)} directory={directory} />
    </Suspense>
  )
}
