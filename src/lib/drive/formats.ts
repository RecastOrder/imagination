 ...
 *
 * 支持程度：
 * - full    可以直接打开（浏览器里完成；Office 由服务器 LibreOffice 转成 PDF）
 * - phase2  后续支持（HEIC / TIFF / RAR / 7z / IFC，需要服务器组件）
 * - phase3  需要商业授权或云服务评估（Revit、SketchUp）
 * - info    不预览，只显示信息并提供下载（DWG 已定为此类）
 */
export type ViewerKey = "pdf" | "office" | "image" | "markdown" | "text" | "table" | "model" | "archive" | "video" | "audio" | "pending"
...
// Office：已定为和其他格式一起完成，只读预览（服务器 LibreOffice 转 PDF）
...
export const FORMATS: Record<string, FormatInfo> = {
  ...
  docx: F("Word 文档", "文档", "office", "full"),
  doc: F("Word 文档（旧版）", "文档", "office", "full"),
  xlsx: F("Excel 表格", "表格", "office", "full"),
  xls: F("Excel 表格（旧版）", "表格", "office", "full"),
  pptx: F("PPT 演示", "文档", "office", "full"),
  wps: F("WPS 文字", "文档", "office", "full"),
  et: F("WPS 表格", "表格", "office", "full"),
  ppt: F("PPT 演示（旧版）", "文档", "office", "full"),
  dps: F("WPS 演示", "文档", "office", "full"),
  rtf: F("RTF 文档", "文档", "office", "full"),
  heic: F("HEIC 照片", "图片", "pending", "phase2", "后续支持：服务器转码为 JPG 后显示（苹果手机默认格式）"),
  tif: F("TIFF 图片", "图片", "pending", "phase2", "后续支持：服务器切成瓦片，支持超大扫描图局部放大"),
  tiff: F("TIFF 图片", "图片", "pending", "phase2", "后续支持：服务器切成瓦片，支持超大扫描图局部放大"),
  rar: F("RAR 压缩包", "压缩包", "pending", "phase2", "后续支持：由服务器解包后，像 ZIP 一样在左侧展开浏览"),
  "7z": F("7z 压缩包", "压缩包", "pending", "phase2", "后续支持：由服务器解包后，像 ZIP 一样在左侧展开浏览"),
  ifc: F("IFC 模型", "三维", "pending", "phase2", "后续支持：用开源 web-ifc 在浏览器中查看构件和属性"),
  ...
  rvt: F("Revit 模型", "三维", "pending", "phase3", "需要云端转换服务；建议先从 Revit 导出 IFC 再上传"),
  skp: F("SketchUp 模型", "三维", "pending", "phase3", "需要转换为 glTF；建议先导出 glTF / OBJ 再上传"),
